 ...
 * 
 * Servicio para guardar y cargar libros en Supabase
 */
...
function validateBookMetadata(metadata: BookMetadata): string | null {
  // Categorías: debe ser un array y no vacío
  ...
  // Géneros: debe ser un array y no vacío
  ...
  if (metadata.autores.length === 0) {
    return "Por favor ingresa al menos un autor";
  }
...
/**
 * Obtiene extensión de archivo desde un File o Blob
 */
 ...
 */
async function fetchFileFromUrl(url: string): Promise<Blob | null> {
  try {
